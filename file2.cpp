...
// Function to log temperature data at regular intervals with alerts
void logTemperatureData(const std::string& filename, int interval_seconds, float temp_threshold) {
    ...
    while (true) {
        try {
            ...
            // Check if the CPU temperature exceeds the threshold and trigger an alert
            if (cpuTemp > temp_threshold) {
                std::cout << "ALERT! CPU Temperature exceeded " << temp_threshold << "°C at " << timestamp << std::endl;
            }
...
int main() {
    ...
    const std::string logFilename = "temperature_log_alert.csv";
    const int intervalSeconds = 5;  // Log every 5 seconds
    const float temperatureThreshold = 65.0;  // Alert threshold for temperature

    try {
        // Start logging temperature data and checking for alerts
        logTemperatureData(logFilename, intervalSeconds, temperatureThreshold);
    } catch (const std::runtime_error& e) {
        std::cerr << "Error: " << e.what() << std::endl;
    }
